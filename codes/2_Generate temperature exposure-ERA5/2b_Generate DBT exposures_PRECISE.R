...
pts <- data.frame(health_facility = c("Illiassa", "Ngeyen Sanjal", "Farafenni", "Mariakani", "Rabai"), 
                  long = c(-15.75143882159046, -15.440109391082256, -15.600228426169666, 39.472804, 39.558933), 
                  lat = c(13.561218867242077, 13.592198240253614, 13.572191626206367,  -3.864297, -3.945486 ))
...
bound1 <- bound0[bound0@data$NAME %in% c("Gambia"), ]
...
for (y in year){
  ...
  df4 <- df3 %>% 
    dplyr::select(-c(ID)) %>% 
    gather(key = date_num, value = tmax, -health_facility)
...
for (y in year){
  ...
  df4 <- df3 %>% 
    dplyr::select(-c(ID)) %>% 
    gather(key = date_num, value = tmin, -health_facility)
...
temp <- tmax %>% 
  ...
  dplyr::select(health_facility, date, year, month, day, tmax, tmin, tmean) 
...
temp <- temp %>% 
  arrange(health_facility, year, month, day) %>% 
  dplyr::select(health_facility, date, year, month, day, tmax, tmin, tmean, dtr, tmax_pct, tmin_pct, tmean_pct, dtr_pct)


save(temp, file = "data/PRECISE/Tmp_data_ERA5.RData")
...
save(apgar_temp, file = "data/PRECISE/apgar_data_for_analysis_DBT_ERA5.RData")
