...
write.csv(results_all, "results/ApgarU7/ERA5_Tmean_pct_heatwave_full_cohort.csv")
...
results_all <- dplyr::bind_rows(results_full, results_bfa, results_gmb,
                                results_ken, results_urban, results_rural)
...
plot_dat <- results_all %>%
  ...
  mutate(
    heatwave   = factor(heatwave,
                        levels = paste0("HW", 1:30)),
    duration   = factor(duration,
                        levels = c("1 day", "2 days", "3 days", "4 days", "5 days")),
    percentile = factor(percentile,
                        levels = c("75th percentile", "80th percentile", "85th percentile",
                                   "90th percentile", "95th percentile", "98th percentile")),
    sample     = factor(sample,
                        levels = c("Full sample", "Burkina Faso", "Gambia",
                                   "Kenya", "Urban", "Rural"))
  )
...
write.csv(results_all, "results/ApgarU7/ERA5_Tmax_pct_heatwave_full_cohort.csv")
...
plot_dat <- results_all %>%
  filter(!is.na(OR), !is.na(LCL), !is.na(UCL)) %>%
  mutate(
    heatwave   = factor(heatwave, levels = paste0("HW", 1:30)),
    duration   = factor(duration,
                        levels = c("1 day", "2 days", "3 days", "4 days", "5 days")),
    percentile = factor(percentile,
                        levels = c("75th percentile", "80th percentile", "85th percentile",
...
write.csv(results_all, "results/ApgarU7/ERA5_Tmin_pct_heatwave_full_cohort.csv")
