...
**Operating systems tested:** *[fill in, e.g. Windows 11 / macOS 14 / Ubuntu 22.04 — see `demo/session_info.R`]*

**R version tested:** *[fill in — run `demo/session_info.R` in your analysis environment and paste the output below]*

**Package versions tested:** see [`software_versions.txt`](software_versions.txt), generated by running `demo/session_info.R`.
...
**(Optional) Reproducing the manuscript's results:** the analyses in `3_Analysis case-crossover/` reproduce all quantitative results reported in the manuscript once run on the linked clinical + ERA5 dataset described in the Methods ("Data and methods") section. Because the individual-level clinical data are restricted, exact reproduction requires requesting access as described in the manuscript's Data Availability statement.
